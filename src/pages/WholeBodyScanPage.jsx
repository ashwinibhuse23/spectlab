...
import { CheckCircle2, AlertCircle, Clock, Activity, Timer, BookOpen, Download } from 'lucide-react';
import { gsap } from 'gsap';

export default function WholeBodyScanPage() {
  useEffect(() => {
    window.scrollTo(0, 0);
  ...
  const prerequisites = [
    '3–4 weeks post total thyroidectomy.',
    'Off medication post surgery.',
    'Avoid Iodine intake 4–5 days prior to the scan.',
  ];
  ...
  const procedureSteps = [
    'Day 1: I-131 solution is given orally.',
    'Day 2: I-131 Whole Body Scan is performed.',
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-12 items-start">
          <div className="space-y-10">
            ...
            <section>
              <div className="flex items-center gap-3 mb-6">
                <BookOpen className="w-7 h-7 text-[#3b7a24]" />
                <h2 className="font-display font-bold text-navy text-lg md:text-xl">Prior the Study</h2>
              </div>
              <ul className="space-y-4 ml-1 md:ml-3">
                {priorStudy.map((item, index) => (
                  <li data-reveal key={index} className="flex items-start gap-4">
                    <div className="w-2 h-2 rounded-full bg-[#3b7a24] mt-2.5 flex-shrink-0" />
                    <p className="text-navy text-[15px] leading-relaxed">{item}</p>
                  </li>
